...
impl PackWriter {
    ...
    pub fn write_pack(
        ...
        log::debug!(target: "fetch", "writing pack file to {:?}", self.git_dir.join("objects").join("pack"));
...
/// Configuration metadata for the [`Fetch`] delegate.
pub struct Config {
    /// The `want`s the client does not have, advertised to the server
    /// side for fetching.
    pub wants: Vec<ObjectId>,
    /// The `have`s the client does have, advertised to the server
    /// side.
    ...
    W: io::Write,
{
    ...
    'negotiation: loop {
        ...
        previous_response = if response.has_pack() {
            log::debug!(target: "fetch", "receiving pack");
            progress.step();
    ...
    loop {
        log::debug!(target: "fetch", "reading to buffer");
        match r.read(&mut buf)? {
            0 => {
                log::debug!(target: "fetch", "no data");
                break;
            }
            n => match std::str::from_utf8(&buf[..n]) {
                Ok(progress) => {
                    ...
                    if lines.into_iter().any(|line| line == "0000") {
                        log::debug!(target: "fetch", "found flush packet");
                        break;
                    }
                }
                Err(e) => {
